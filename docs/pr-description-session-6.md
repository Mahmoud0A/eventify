...
5. **Deployment prep** — `docs/deployment.md` for Render (API; worker is paid-only trade-off), Neon Postgres (`npx prisma migrate deploy`), Upstash Redis, dashboard env vars. Accounts not provisioned; no live URL claimed.
